...
if path.exists(os.environ['ONDEMAND_HOME']+'/conf/local.json'):
    with open(os.environ['ONDEMAND_HOME']+'/conf/local.json') as json_file:
        sample_table = json.load(json_file)[0]
else:
    print("WARNING: $ONDEMAND_HOME/conf/local.json not found! Won't be able to resolve file paths") 
